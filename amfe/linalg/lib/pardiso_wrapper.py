...
class PardisoWrapper(object):
    ...
    def __init__(self, A, mtype=11, iparm={}, verbose=False):
        ...
        iparm: dictionary with iparm values to override after initialization with mtype
            ...
            iparm[5] : {0: The array x contains the solution (default), 1: The solver stores the solution on the RHS b}
            ...
            iparm[7] : Iterative Refinement step (maximum number) {0: The solver automatically performs 2 steps when
                        perturbed pivots are obtained during factorization, >0: maximum number of iterative refinement
                        steps, <0 same as >0 but the accumulation of residuum uses extended precision data types}
            iparm[8] : Reserved set to zero
            iparm[9] : Pivoting perturbation, small pivots are perturbed with eps = 10**(-iparm[9])
                       default value for nonsymmetric matrices: 13, default value for symmetric indefinite matrices: 8
            ...
            iparm[12] : Use symmetric weighted matching {0: Disable matching (default for symmetric indefinite matrices)
            ...
            iparm[18] : input/output: Report number of floating point operations in 10^6 floating point operations that are
                        necessary to factorize A {<0: Enable Report (this increases reordering time), >0: disable report
            ...
            iparm[23] : Parallel factorization control {0: classic algorithm (default), 1:two-level factorization
                        ...
                        this can only be used in conjunction with some certain iparm values for the other iparm param.
            ...
            iparm[30] : Partial solve: This parameter controls the solve step. It can be used if only few components of
                        solution vector are needed
                        {0: disable (default), 1: Use the perm vector to identify the elements that are zero on RHS,
                        2: Assume only few are nonzero, but make full computation => set rhs to zero values where not
                        needed,
            ...
            iparm[36] : Storage format: {0: Use CSR-Format (default), 1: use BSR format, -1: use variable BSR format}
            ...
            iparm[55] : Diagonal pivoting control: {0: internal function used to work with pivot and calculation
                        ...
                        1: You can use mkl_pardiso_pivot callback routine to control pivot elements whcih appear during
